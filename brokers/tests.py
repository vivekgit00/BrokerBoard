...
class DeltaRestClient:
    ...
    def request(self, method, path, payload=None, query=None, auth=False, base_url=None, headers={}):
        ...
        if auth:
            ...
            headers = {"Content-Type": "application/json", "api-key": self.api_key, "timestamp": timestamp,
                       "signature": signature, "User-Agent": "delta-rest-client-v" + str('1.0.12')}

            res = self.session.request(
                method, url, data=body_string(payload), params=query, timeout=(3, 6), headers=headers
            )
        else:
            non_auth_headers = {'User-Agent': 'delta-rest-client-v%s' % "1.0.12", 'Content-Type': 'application/json'}
            res = requests.request(method, url, data=body_string(payload), params=query, timeout=(3, 6),
    ...
    def place_stop_order(self, product_id, size, side, stop_price=None, limit_price=None, trail_amount=None,
                         order_type=OrderType.LIMIT, isTrailingStopLoss=False):
        order = {
            'product_id': product_id,
            'size': int(size),
            'side': side,
    ...
    def place_order(self, product_id, size, side, limit_price=None, time_in_force=None, order_type=OrderType.LIMIT,
                    post_only='false', client_order_id=None, reduce_only='false'):
        order = {
            'product_id': product_id,
            'size': int(size),
            'side': side,
